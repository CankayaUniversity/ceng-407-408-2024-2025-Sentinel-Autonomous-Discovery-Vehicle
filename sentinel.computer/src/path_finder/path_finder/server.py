...
from message_interfaces.srv import OdomToImage
...
class PathFinder(Node):
    ...
    def __init__(self):
        ...
        self.srv = self.create_service(
            OdomToImage, "odom_to_image", self.handle_odom_to_image
        )
        self.map_data = None
    ...
    def create_map_subscription(self):
        if self.map_subscription is None:
            ...
            self.map_subscription = self.create_subscription(
                OccupancyGrid, "/map", self.map_callback, 10  # QoS profile depth
            )
    ...
    def handle_odom_to_image(self, request, response):
        ...
        if self.map_data is None:
            ...
            wait_count = 0
            max_wait_count = (
                100  # Increased from 30 to 100 (10 seconds with 10Hz polling)
            )
            ...
            while self.map_data is None and wait_count < max_wait_count:
                if wait_count % 10 == 0:  # Log status every 1 second
                    self.get_logger().info(
                        f"Waiting for map data... ({wait_count}/{max_wait_count})"
                    )
            ...
            if self.map_data is None:
                self.get_logger().error(
                    f"Failed to receive map data within {elapsed:.2f} seconds"
                )
                response.image_data = ""
                return response
            else:
                self.get_logger().info(
                    f"Successfully received map after {elapsed:.2f} seconds"
                )

        odom_data = request.odom
        self.get_logger().info(
            f"Processing odometry at position: ({odom_data.pose.pose.position.x}, {odom_data.pose.pose.position.y})"
        )
    ...
    def process_path_planning(self, map_data, odom_data):
        ...
        self.get_logger().info(
            f"Planning with robot dimensions: {robot_width}m x {robot_length}m"
        )

        # Create occupancy grid
        grid, resolution, origin, filtered_grid, initial_grid = (
        ...
        goal_world = (odom_data.pose.pose.position.x, odom_data.pose.pose.position.y, 0)
        ...
        if original_start != start_grid:
            self.get_logger().info(
                f"Start point adjusted from {original_start} to {start_grid}"
            )
        if original_goal != goal_grid:
            self.get_logger().info(
                f"Goal point adjusted from {original_goal} to {goal_grid}"
            )
    ...
    def create_occupancy_grid(
        self, map_data, robot_width=0.25, robot_length=0.24, min_obstacle_size=3
    ):
    ...
    def generate_visualization(self, map_data, path, start, goal):
        ...
        colors = [
            (200 / 255, 200 / 255, 210 / 255),  # unknown (-1)
