...
namespace sentinel_hardware_interface
{
    ...
    hardware_interface::CallbackReturn SentinelHardwareInterface::on_init(const hardware_interface::HardwareInfo &info)
    {
        ...
        std::string right_front_wheel = info_.hardware_parameters["right_front_wheel"];
    ...
    std::vector<hardware_interface::StateInterface> SentinelHardwareInterface::export_state_interfaces()
    {
    ...
    std::vector<hardware_interface::CommandInterface> SentinelHardwareInterface::export_command_interfaces()
    {
        std::vector<hardware_interface::CommandInterface> command_interfaces;

        for (auto &left_wheel : this->left_wheels)
        {
            command_interfaces.emplace_back(hardware_interface::CommandInterface(left_wheel.name, hardware_interface::HW_IF_VELOCITY, &left_wheel.cmd));
        }
        for (auto &right_wheel : this->right_wheels)
        {
            command_interfaces.emplace_back(hardware_interface::CommandInterface(right_wheel.name, hardware_interface::HW_IF_VELOCITY, &right_wheel.cmd));
        }

        return command_interfaces;
    }

    hardware_interface::CallbackReturn SentinelHardwareInterface::on_configure(const rclcpp_lifecycle::State &)
    {
        RCLCPP_INFO(rclcpp::get_logger("SentinelHardwareInterface"), "Configuring");
        RCLCPP_INFO(rclcpp::get_logger("SentinelHardwareInterface"), "Successfully configured!");
        return hardware_interface::CallbackReturn::SUCCESS;
    }

    hardware_interface::CallbackReturn SentinelHardwareInterface::on_cleanup(const rclcpp_lifecycle::State & /*previous_state*/)
    {
        RCLCPP_INFO(rclcpp::get_logger("SentinelHardwareInterface"), "Cleaning up");
        RCLCPP_INFO(rclcpp::get_logger("SentinelHardwareInterface"), "Successfully cleaned up");
        return hardware_interface::CallbackReturn::SUCCESS;
    }

    hardware_interface::CallbackReturn SentinelHardwareInterface::on_activate(const rclcpp_lifecycle::State & /*previous_state*/)
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("SentinelHardwareInterface"), "Successfully activated");
        return hardware_interface::CallbackReturn::SUCCESS;
    }

    hardware_interface::CallbackReturn SentinelHardwareInterface::on_deactivate(const rclcpp_lifecycle::State & /*previous_state*/)
    {
        ...
        RCLCPP_INFO(rclcpp::get_logger("SentinelHardwareInterface"), "Successfully deactivated");
        return hardware_interface::CallbackReturn::SUCCESS;
    }

    hardware_interface::return_type SentinelHardwareInterface::read(const rclcpp::Time & /*time*/, const rclcpp::Duration &period)
    {
        if (!this->raspberry_pi_commands.connected())
        {
        ...
        this->raspberry_pi_commands.read_encoder_values(this->left_wheels[0].encoder, this->right_wheels[0].encoder);
    ...
    hardware_interface::return_type SentinelHardwareInterface::write(const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
    {
        ...
        int left_cmd_value = left_cmd / this->left_wheels[0].rads_per_count / this->loop_rate / 0.0058;
        int right_cmd_value = right_cmd / this->right_wheels[0].rads_per_count / this->loop_rate / 0.0058;

        this->raspberry_pi_commands.set_motor_values(static_cast<int>(left_cmd_value), static_cast<int>(right_cmd_value));
...
PLUGINLIB_EXPORT_CLASS(
    sentinel_hardware_interface::SentinelHardwareInterface, hardware_interface::SystemInterface)