...
void UdpSocket::configure_computer()
{
    ...
    if (inet_pton(AF_INET, this->computer_host.c_str(), &this->computer_addr.sin_addr) <= 0)
    {
...
void UdpSocket::send(void *data, int size, int id)
{
    ...
    std::string base64_frame = base64::to_base64(std::string_view(static_cast<const char *>(data), size));

    size_t total_size = base64_frame.size();

    for (size_t offset = 0; offset < total_size; offset += this->chunk_size)
    {
        size_t send_size = std::min(static_cast<size_t>(this->chunk_size), total_size - offset);
        ...
        {
            throw std::runtime_error("Partial data sent. Expected: " +
